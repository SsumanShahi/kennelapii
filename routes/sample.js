...
// const Sample = require('../models/dogregister');
...
router.get ('/dog', auth.verifyUser, (req,res,next) =>{
...
router.get('/alldogs', (req, res, next) => {
    DogRegister.find()
        .then((dogregister) => {
            res.json(dogregister);
        }).catch(next);
...
.post('/postrouter', auth.verifyUser, (req,res,next) =>{
